...
function setupAuth() {
    ...
    setEnv('JWT_PRIVATE_KEY', privateKey); // Specifically fix the missing JWT_PRIVATE_KEY error
    setEnv('JWKS', jwks);

    const adapterSecret = crypto.randomBytes(32).toString('base64');
    setEnv('CONVEX_AUTH_ADAPTER_SECRET', adapterSecret);

    console.log('Auth environment variables setup complete.');
}
